"""
2 Q-Networks SAC CNN-only (No SpatioTemporal Transformer)
...
Usage:
    python 2qCNNsac_experiment.py --timesteps 100000 --render
    python 2qCNNsac_experiment.py --timesteps 50000 --log-dir ./logs/2q_cnn_v1
"""
...
def create_cnn_env(
    ...
    Args:
        use_advanced_rewards: Enable 5 advanced reward shaping mechanisms
            (lane centering, forward motion, safe following, traffic flow, yield & maneuver)
    """
    ...
    # Enable/Disable advanced reward shaping (NEW)
    env._use_advanced_rewards = use_advanced_rewards
    if use_advanced_rewards:
        print("[ENV] Advanced reward shaping ENABLED (lane center, forward, headway, flow, yield)")
    else:
        print("[ENV] Advanced reward shaping DISABLED (base rewards only)")
    
    # Get map for waypoint logging
    carla_map = env.world.get_map()
    #carla_map.reload_world() 
...
def train_2q_cnn_sac(
    ...
    print("=" * 80)
    print("2 Q-NETWORK SAC WITH CNN ONLY (NO SPATIOTEMPORAL TRANSFORMER)")
    print("=" * 80)
    ...
    print(f"Advanced Rewards: {'ENABLED' if use_advanced_rewards else 'DISABLED'}")
    ...
    policy_kwargs = {
        ...
        'n_critics': 2,          # 2 Q-networks (standard)
    ...
    trust_callback = TrustScoreCallback(update_freq=100, verbose=1)
    ...
    try:
        model.learn(
            ...
            log_interval=10,
            callback=[checkpoint_callback, safety_callback, trust_callback],
            progress_bar=True
        )
    ...
    # Save final model
    final_path = os.path.join(log_dir, "sac_2q_cnn_final.zip")
    ...
    with open(info_path, "w") as f:
        f.write("2 Q-Network SAC with CNN Only (No SpatioTemporal Transformer)\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write(f"\nKey Difference from STT Variants:\n")
        f.write(f"- Direct RGB image input to CNN policy\n")
        f.write(f"- No SpatioTemporal Transformer encoding\n")
        f.write(f"- Faster inference, simpler architecture\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="2 Q-Network SAC with CNN Only"
    )
    parser.add_argument("--timesteps", type=int, default=100000, help="Total timesteps")
    parser.add_argument("--log-dir", type=str, default="./logs/2q_cnn", help="Log directory")
