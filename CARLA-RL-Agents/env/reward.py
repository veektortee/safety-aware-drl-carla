...
class Reward:
    def __init__(self) -> None:
        ...
        # Unified time-tracking for behavioral penalties
        self.zero_speed_time     = 0.0   # Accumulates time at near-zero speed [seconds]
        self.opposite_lane_time  = 0.0   # Accumulates time in opposite lane [seconds]
        self.TIMESTEP            = 0.01  # CARLA simulation timestep (100 steps/sec)
        # Safety constraint tracking
        self.cbf_correction_magnitude = 0.0  # L2 norm of last action correction by CBF
        self.red_light_violations = 0  # Count of red light crossings
        self.green_light_compliance = 0  # Count of steps moving on green
        self.reward_log = {}  # Store reward breakdown for logging
        
        self.countint = 0

    # ======================================== Main Reward Function ==========================================================
    def calculate_reward(self, vehicle: Vehicle, current_pos, target_pos, next_waypoint_pos, speed, world=None, cbf_correction_magnitude=0.0) -> float:   
        target_distance = self.distance(current_pos, target_pos)
        next_waypoint_distance = self.distance(current_pos, next_waypoint_pos)
        self.cbf_correction_magnitude = cbf_correction_magnitude  # Store for safety penalty
        ...
        reward = self.__collision_reward(vehicle) + \
            ...
            self.__zero_speed_penalty(speed) + \
            self.__opposite_lane_penalty(vehicle) + \
            self.__cbf_safety_penalty() + \
            self.__traffic_light_penalty(vehicle, world) + \
    ...
    def __collision_reward(self, vehicle):
        '''
        This reward function penalizes the vehicle if it collides with anything. Lane invasion is now tracked via __opposite_lane_penalty().
        {
            0.0     : if no collision occurred,
            -lambda : if collision occurred
        }
        
        Based on the calculations, the max reward for this function is 0 and the min reward is -20;
        lambda = 20
        '''
        lbd = 20
    ...
    def __zero_speed_penalty(self, speed, speed_threshold=1.0):
        '''
        Logarithmic penalty for prolonged idling (unified time-tracking pattern used by multiple penalties).
        {
            0                                : if speed >= speed_threshold,
            -log(1 + zero_speed_time) * lbd : if speed < speed_threshold
        }
        Threshold: 1.0 km/h (tuned for continuous SAC/PPO agents)
        '''
        lbd = 2.0 / config.ENV_MAX_STEPS
        
        if speed < speed_threshold:
            self.zero_speed_time += self.TIMESTEP
            penalty = -np.log(1.0 + self.zero_speed_time) * lbd
            return penalty
        else:
            self.zero_speed_time = 0.0
            return 0.0
    
    def __opposite_lane_penalty(self, vehicle):
        '''
        Logarithmic penalty for time spent in opposite lane (mirrors __zero_speed_penalty pattern).
        Accumulates over time, encouraging agent to return to correct lane quickly.
        {
            0                                  : if not in opposite lane,
            -log(1 + opposite_lane_time) * lbd : if in opposite lane
        }
        '''
        lbd = 1.5 / config.ENV_MAX_STEPS  # Slightly lower scaling than zero speed (1.5 vs 2.0)
        
        if vehicle.lane_invasion_occurred():
            self.opposite_lane_time += self.TIMESTEP
            penalty = -np.log(1.0 + self.opposite_lane_time) * lbd
            return penalty
        else:
            self.opposite_lane_time = 0.0
            return 0.0

    def __cbf_safety_penalty(self):
        '''
        Penalty for CBF corrections (safety constraint violations).
        Severity-based: higher correction magnitudes incur larger penalties.
        When CBF layer invokes: -penalty proportional to correction magnitude^1.5
        Discourages unsafe policies that require strong safety interventions.
        {
            0.0  : if correction_magnitude < 0.01 (noise threshold),
            -(correction_magnitude^1.5) * scale : otherwise
        }
        '''
        min_correction_threshold = 0.01  # Ignore tiny corrections (numerical noise)
        scale = 0.5 / config.ENV_MAX_STEPS  # Scales to ~0.5 penalty per step if constantly correcting
        
        if self.cbf_correction_magnitude > min_correction_threshold:
            # Severity scales with correction magnitude (higher = worse)
            penalty = -(self.cbf_correction_magnitude ** 1.5) * scale
            return penalty
        return 0.0

    def __traffic_light_penalty(self, vehicle, world=None):
        '''
        Reward/penalty based on traffic light state (safety constraint).
        PENALTY: Large -20 if running red light (critical safety violation)
        REWARD: +0.3 if moving on green (safe progress), +0.4 if stopped on red (safe compliance)
        Logged: red_light_violations, green_light_compliance
        {
            -20.0 : if running red light,
            +0.4  : if stopped on red light,
            +0.3  : if moving on green light,
            0.0   : otherwise
        }
        '''
        if world is None:
        ...
        try:
            ...
            # Get traffic lights affecting this waypoint
            ...
            for traffic_light in traffic_lights:
                ...
                if state == carla.TrafficLightState.Red:
                    ...
                    for stop_waypoint in stop_waypoints:
                        distance_to_stop = current_waypoint.transform.location.distance(stop_waypoint.transform.location)
                        
                        # VIOLATION: Crossing red light while moving
                        if distance_to_stop < 2.0 and current_speed > 1.0:  # > 1 m/s
                            self.red_light_violations += 1
                            return -20.0  # Large penalty for critical safety violation
                        
                        # COMPLIANCE: Stopped before stop line on red
                        elif distance_to_stop < 10.0 and current_speed < 0.5:
                            self.green_light_compliance += 1
                            return 0.4  # Reward for safe compliance
                
                # GREEN LIGHT: Reward for making progress
                elif state == carla.TrafficLightState.Green:
                    if current_speed > 1.0:  # Moving
                        self.green_light_compliance += 1
                        return 0.3  # Reward for safe progress
                    
        except Exception as e:
            # Silently fail if traffic light check fails (not all roads have lights)
            pass
    ...
    def reset(self, waypoints):
        ...
        self.total_ep_reward  = 0
        self.zero_speed_time  = 0.0   # Reset time-based penalties
        self.opposite_lane_time = 0.0 # Reset lane invasion timer
