...
class PreProcessing:
    ...
    def preprocess_data(self, observation_data):
        ...
        Processes RGB, Lidar point clouds, and scalar state features.
        '''
        ...
        if 'lidar_data' in observation_data and observation_data['lidar_data'] is not None:
            neo_observation_data['lidar_data'] = self.__process_lidar(observation_data['lidar_data'])
        else:
            # Provide zero padding if lidar not available
            neo_observation_data['lidar_data'] = np.zeros((3, 500), dtype=np.float32)
