...
class Vehicle:
    def __init__(self, world):
        ...
        self.__previous_steer = 0.0 # For steering rate limiting (smooth control)
    ...
    def control_vehicle(self, action):
        # Apply gentle steering rate limiting to reduce yanking behavior after lane invasion
        max_steering_rate = 0.1  # Max steering change per step (rad/step)
        target_steer = max(-1.0, min(float(action[0]), 1.0))
        self.__control.steer = np.clip(
            target_steer,
            self.__previous_steer - max_steering_rate,
            self.__previous_steer + max_steering_rate
        )
        self.__previous_steer = self.__control.steer
        ...
        # Sync steering angle from the control command to keep internal state in sync with CARLA
        self.__steering_angle = self.__control.steer
